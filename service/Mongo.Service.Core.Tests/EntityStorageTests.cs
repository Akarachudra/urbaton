...
{
    [TestFixture]
    public class EntityStorageTests : TestsBase
    {
        ...
        public void CanReadWithSkipAndLimit()
        {
            ...
            anonymousEntitiesBefore = entities.Reverse()
                                              ...
                                              .Skip(1)
                                              .Take(2);
            readedEntities = storage.Read(x => !string.IsNullOrEmpty(x.SomeData), 1, 2, x => x.SomeData, true);
            anonymousEntitiesAfter = readedEntities.Select(x => new { x.Id, x.SomeData });
            ...
            anonymousEntitiesBefore = entities.Select(x => new { x.Id, x.SomeData })
                                              .Skip(1)
                                              .Take(2);
            readedEntities = storage.Read(x => !string.IsNullOrEmpty(x.SomeData), 1, 2, x => x.SomeData);
            anonymousEntitiesAfter = readedEntities.Select(x => new { x.Id, x.SomeData });
            CollectionAssert.AreEqual(anonymousEntitiesBefore, anonymousEntitiesAfter);
        }
        ...
        public void TestMultithreadedSyncedWriteRead()
        {
            ...
            var maxReadTime = TimeSpan.FromSeconds(40);
            do
            {
                SampleEntity[] entities;
                SampleEntity[] deletedEntities;
